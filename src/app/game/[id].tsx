...
const GameDetails = () => {
    ...
    const checkFavorite = async () => {
        try {
            const response = await fetch(`http://10.0.2.2:3000/favorites/check-favorite/${session.user.id}/${id}`)
            if (!response.ok) {
                throw new Error('Network response was not ok')
            }
            const result = await response.json()
    ...
    const handleFavorite = async () => {
        try {
            const response = await fetch('http://10.0.2.2:3000/favorites/send-favorite', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    const handleUnfavorite = async () => {
        try {
            const response = await fetch(`http://10.0.2.2:3000/favorites/delete-favorite/${id}/${session.user.id}`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    return (
        <View className="h-full flex flex-col flex-1">
                    ...
                    <View className="flex-1 justify-center items-center bg-black/50 ">
                        <View className="bg-zinc-800 rounded-lg p-6 w-80">
                            ...
                            <View className="items-center pb-6">
                                <Text className="text-lg mb-4 text-white">Como você avalia esse jogo?</Text>
                                <StarRating
                                    rating={starRating}
            ...
            <Pressable className="absolute bottom-6 right-6 flex flex-row gap-2 p-3 items-center justify-center border-2 border-green-500 rounded-xl"
